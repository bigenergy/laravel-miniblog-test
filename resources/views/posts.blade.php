...
<div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
    @if (Route::has('login'))
        <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
            @auth
                <a href="{{ url('/dashboard') }}" class="text-sm text-gray-700 dark:text-gray-500 underline">Дашборд</a>
            @else
                        ...
                        @forelse($posts as $post)
                            <div class="card">
                                <div class="card-body">
                                    <h2>{{ $post->title }}</h2>
                                    <p>{{ $post->content }}</p>
                                    <small>Автор: {{ $post->user->name }} | Создан: {{ $post->created_at }}</small>
                                </div>
                            </div>
                        @empty
                            <b>В этом блоге еще нет постов</b>
                        @endforelse
                    </div>
                </div>
            </div>
